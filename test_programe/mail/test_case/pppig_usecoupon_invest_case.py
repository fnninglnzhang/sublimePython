...
class To_investTest(myunit.MyTest):
    ...
    def test_to_invest_success(self):
        ...
        try:
            ...
            sleep(2)
            po.open()
            po.pppiglogin_noclose_Action("13335726549", "111111")            # 用户登陆
            sleep(2)
            po.open_R('/recommendloanDetail?loanId=35414')                 # 标的 URL
            po1 = To_invest(self.driver)
            sleep(2)
            po1.pppiguse_ratecoupon_Invest_Action('6200')                         # 投资金额   使用加息券
